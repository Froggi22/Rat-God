...
export async function run (interaction) {
	...
	function pushData (item) {
		...
		tableData.push([ // Add the bullet's data as a new row
			...
			item._props.ProjectileCount > 1 ? item._props.Damage * item._props.ProjectileCount : item._props.Damage,
			item._props.ArmorDamage,
			Math.floor(item._props.FragmentationChance * 100),
			item._props.InitialSpeed
		])
	}

	const tableData = []
	for (let objNr = 0; objNr < ammoFetch.length; objNr++) { // Iterate through each ammo object
		const item = ammoFetch[objNr]
		if (!item._props.Caliber) continue
		if (item._props.Caliber.replace("Caliber", "") === caliber) pushData(item)
		else if (caliber === "127x108" && item._props.Caliber.replace("Caliber", "") === "30x29") pushData(item) // The caliber 127x108 includes 30x29
	}
	...
	function sortTable (statSorting) {
		...
		tableData.sort((x, y) =>
			y[statPos[0]] - x[statPos[0]] ||
			y[statPos[1]] - x[statPos[1]] ||
			y[statPos[2]] - x[statPos[2]]
		)
	}
