...
export async function run (interaction) {
	if (!tarkovServiceStatus?.fetched || Date.now() - tarkovServiceStatus.fetched > 60 * 60 * 1000) tarkovServiceStatus = await fetchStatus() // If data is undefined or older than an hour, fetch again

	if (!tarkovServiceStatus?.fetched) { // If fetching was unsuccessful, reply with an error response
...
export async function fetchStatus () {
	// const serviceStatus = JSON.parse(await readFile("tarkovServiceStatus.json")) // Used for mass-testing/restarting when you don't want to spam requests to the API. Uncomment this code line and comment the fetch. Open the API URL in your browser, save as json file in project directory.
